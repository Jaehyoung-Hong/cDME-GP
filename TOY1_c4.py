...
def train_step(dataloader,
    ...
    for i, (X, y, _) in enumerate(dataloader):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            tape.watch(get_mean_params(gp1)+get_mean_no_embed_params(gp2)+get_mean_no_embed_params(gp3)+get_mean_no_embed_params(gp4))                
        ...
        grads = tape.gradient(loss_step, get_mean_params(gp1)+get_mean_no_embed_params(gp2)+get_mean_no_embed_params(gp3)+get_mean_no_embed_params(gp4))
        optimizer_train_mean.apply_gradients(zip(grads,get_mean_params(gp1)+get_mean_no_embed_params(gp2)+get_mean_no_embed_params(gp3)+get_mean_no_embed_params(gp4)))
...
def evaluate(data_loader,n_adapt,lr_val,ker_val1,mean_val1,noise_val1,ker_val2,mean_val2,noise_val2,ker_val3,mean_val3,noise_val3,ker_val4,mean_val4,noise_val4):
    ...
    return rmse1,rmse2,rmse3,my_cluster1,my_cluster2,my_cluster3,my_cluster4
...
if __name__ == '__main__':
    ...
    for n_iter in [1,7]:
        ...
        model3 = MLP(32,1)
        mean3 = Warping_mean(model3, model_embed)

        model4 = MLP(32,1)
        mean4 = Warping_mean(model4, model_embed)
        ...
        kernel3 = RBF_embed(amplitude=amplitude3,length_scale=length_scale3,embed_fn=model_embed)
        kernel4 = RBF_embed(amplitude=amplitude4,length_scale=length_scale4,embed_fn=model_embed)
        ...
        best_mean1.save_weights(f"./TOY{n_iter}/mean_c4_1_ckpt")
        best_mean2.save_weights(f"./TOY{n_iter}/mean_c4_2_ckpt")
        best_mean3.save_weights(f"./TOY{n_iter}/mean_c4_3_ckpt")
        best_mean4.save_weights(f"./TOY{n_iter}/mean_c4_4_ckpt")

        last_loss1,last_loss2,last_loss3,test_cluster1,test_cluster2,test_cluster3,test_cluster4=evaluate(test_tensor,250,lr_val,eval_ker1,eval_mean1,eval_noise1,eval_ker2,eval_mean2,eval_noise2,eval_ker3,eval_mean3,eval_noise3,eval_ker4,eval_mean4,eval_noise4)
        
        with open(f'./TOY{n_iter}/c4Cluster.txt', 'wb') as f:
            pickle.dump((best_cluster1,best_cluster2,best_cluster3,best_cluster4,test_cluster1,test_cluster2,test_cluster3,test_cluster4), f)
    ...
    print(f"Std of amplitude3 = {np.std(amplitude_set3)}")
    print(f"Mean of amplitude4 = {np.mean(amplitude_set4)}")
    print(f"Std of amplitude4 = {np.std(amplitude_set4)}")
    ...
    print(f"Std of length3 = {np.std(length_set3)}")
    print(f"Mean of length4 = {np.mean(length_set4)}")
    print(f"Std of length4 = {np.std(length_set4)}")
    ...
    print(f"Std of noise3 = {np.std(noise_set3)}")   
    print(f"Mean of noise4 = {np.mean(noise_set4)}")
    print(f"Std of noise4 = {np.std(noise_set4)}")

    with open('./TOY/c4Loss_Review.txt', 'wb') as f:
        pickle.dump((last_set1,last_set2,last_set3,amplitude_set1,amplitude_set2,amplitude_set3,amplitude_set4,length_set1,length_set2,length_set3,length_set4,noise_set1,noise_set2,noise_set3,noise_set4), f)
