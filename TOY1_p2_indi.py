...
def train_step(dataloader,
    ...
    for i, (X, y, _) in enumerate(dataloader):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            tape.watch(get_mean_params(gp1)+get_mean_params(gp2))                
        ...
        grads = tape.gradient(loss_step, get_mean_params(gp1)+get_mean_params(gp2))
        optimizer_train_mean.apply_gradients(zip(grads,get_mean_params(gp1)+get_mean_params(gp2)))
...
def evaluate(data_loader,n_adapt,lr_val,ker_val1,mean_val1,noise_val1,ker_val2,mean_val2,noise_val2):
    ...
    for (X, y, Z) in tqdm(data_loader):
        ...
        for _ in range(n_adapt):
            with tf.GradientTape(watch_accessed_variables=False) as tape:
                variables = gp_new1.trainable_variables
                tape.watch(variables)
                loss_step1 = -gp_new1.log_prob(train_y)
            grads = tape.gradient(loss_step1, variables)
            optimizer_gp_val1.apply_gradients(zip(grads,variables))

        for _ in range(n_adapt):
            with tf.GradientTape(watch_accessed_variables=False) as tape:
                variables = gp_new2.trainable_variables
                tape.watch(variables)                
                loss_step2 = -gp_new2.log_prob(train_y)
            grads = tape.gradient(loss_step2, variables)
            optimizer_gp_val2.apply_gradients(zip(grads,variables))
...
if __name__ == '__main__':
    ...
    for n_iter in range(10):
        ...
        model_embed1 = MLP_embed(16)
        model1 = MLP(32,1)
        mean1 = Warping_mean(model1, model_embed1)

        model_embed2 = MLP_embed(16)
        model2 = MLP(32,1)
        ...
        with open(f'./TOY{n_iter}/p2Cluster.txt', 'wb') as f:
    ...
    with open('./TOY/p2Loss_indi.txt', 'wb') as f:
